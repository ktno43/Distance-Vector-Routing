...
public class ServerThread extends Thread {
	...
	public static final Long TIMEOUT = (long) 5000000;
	...
	protected void read(byte[] msg, int bytesRead) {
		try {
			...
			for (int j = 8, k = 1; j < (10 * updateFields); j += 10, k++) {
				...
				if (ipN.equals(this.Ip) && (getNodeByID(nID) != this.serverNode) && nCost != 65535
						&& (nCost < this.rtMap.get(getNodeByIpAndPort(senderIp, senderPort)))) {
					updateCost(this.serverId, getNodeByIpAndPort(senderIp, senderPort).getID().intValue(),
							Integer.toString(nCost));

					this.hopMap.put(getNodeByIpAndPort(senderIp, senderPort), this.serverNode);
				}

				for (int i = 0; i < rtMap.size(); i++) {
					if (nCost != 0 && nCost < rtMap.get(getNodeByIpAndPort(ipN, nPort))) {
